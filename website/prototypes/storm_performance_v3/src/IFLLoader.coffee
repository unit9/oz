class IFLLoader
    ...
    parseLibrary:(data) ->
        ...
        console.log "[#{@url}] decompression time: #{time} , raw data length: #{data.length}" 
    ...
    onLibraryParsed:(data) ->
        ...
        time = (new Date().getTime() - @t)/1000
        console.log "[#{@url}] Library Parse time: #{time}" 
    ...
    convertMesh: (iflmesh) ->
        ...
        # if positions.length < 65535 and !isSkinnedMesh and iflmesh.subMeshes.length == 1 and !colors?
    ...
    getTexture: (id) ->
        ...
        if !tex?
            console.warn "[#{@url}] Texture #{id} not found"
            return null
        ...
        if tex.converted?
            ...
            console.warn "[#{@url}] Texture #{id} was not converted"   
