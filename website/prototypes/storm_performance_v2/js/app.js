...
(function() {
  var App, IFLLoader, IFLPhongFresnelShader, IFLStormParticleShader,
  ...
  App = (function() {
    ...
    function App() {
      ...
      this.cleanPlanes = __bind(this.cleanPlanes, this);

      this.clean = __bind(this.clean, this);

      this.getFresnelShader = __bind(this.getFresnelShader, this);

      this.getMaterial = __bind(this.getMaterial, this);

      this.addPlanes = __bind(this.addPlanes, this);
    ...
    App.prototype.onTerrainLoaded = function(iflscene) {
      var gui, particleNumberController, particleSizeController, planesNumberController,
        _this = this;
      this.mesh = iflscene.children[0].children[0];
      ...
      this.addPlanes();
      gui = new dat.GUI;
      gui.add(this.particles, "sortParticles");
      particleNumberController = gui.add(this, "numberOfParticles", 100, this.mesh.geometry.vertices.length);
      planesNumberController = gui.add(this, "numberOfPlanes", 1, this.mesh.geometry.vertices.length);
      particleSizeController = gui.add(this, "particleSize", 5, 20);
      particleNumberController.onFinishChange(function(value) {
        _this.clean();
        _this.numberOfParticles = parseInt(_this.numberOfParticles);
        return _this.addParticles();
      });
      planesNumberController.onFinishChange(function(value) {
        _this.cleanPlanes();
        _this.numberOfPlanes = parseInt(_this.numberOfPlanes);
        return _this.addPlanes();
      });
      return particleSizeController.onFinishChange(function(value) {
        var i, _i, _ref, _results;
        _results = [];
        for (i = _i = 0, _ref = _this.attributes.size.value.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
          _results.push(_this.attributes.size.value[i] = Math.random() * parseInt(value));
        }
        return _results;
      });
    };
    ...
    App.prototype.addParticles = function() {
      var c, geometry, shaderMaterial, v, _i, _j, _ref, _ref1;
      shaderMaterial = this.getMaterial();
      geometry = new THREE.Geometry();
      for (v = _i = 0, _ref = this.numberOfParticles; 0 <= _ref ? _i < _ref : _i > _ref; v = 0 <= _ref ? ++_i : --_i) {
        geometry.vertices.push(this.mesh.geometry.vertices[v]);
      }
      this.particles = new THREE.ParticleSystem(geometry, shaderMaterial);
      this.particles.dynamic = true;
      this.particles.scale.set(1.0, -1.0, 1.0);
      this.particles.sortParticles = true;
      this.particles.position.y = -5;
      for (v = _j = 0, _ref1 = this.particles.geometry.vertices.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; v = 0 <= _ref1 ? ++_j : --_j) {
        this.attributes.size.value[v] = Math.random() * 5;
        this.attributes.customColor.value[v] = new THREE.Color(0xffaa00);
        c = 0.1 + 0.1 * (v / this.particles.geometry.vertices.length);
        this.attributes.customColor.value[v].setRGB(c, c, c);
      }
      return this.scene.add(this.particles);
    };

    App.prototype.addPlanes = function() {
      var geom, mat, pl, vertex, _i, _ref, _results;
      this.planes = new THREE.Object3D;
      this.planes.matrixAutoUpdate = false;
      this.planes.position.y = -5;
      this.planes.rotation.x = Math.PI;
      this.scene.add(this.planes);
      geom = new THREE.PlaneGeometry(4.8, 4.8);
      mat = this.getFresnelShader();
      _results = [];
      for (vertex = _i = 0, _ref = this.numberOfPlanes; 0 <= _ref ? _i <= _ref : _i >= _ref; vertex = 0 <= _ref ? ++_i : --_i) {
        pl = new THREE.Mesh(geom, mat);
        pl.growSpeed = 0.5 + Math.random();
        pl.matrixAutoUpdate = false;
        pl.position.copy(this.mesh.geometry.vertices[vertex]);
        pl.updateMatrix();
        _results.push(this.planes.add(pl));
      }
      return _results;
    };

    App.prototype.getMaterial = function() {
      var shaderMaterial;
      this.attributes = {
        size: {
          type: 'f',
      ...
      this.uniforms = {
        ...
        texture: {
          type: "t",
          value: THREE.ImageUtils.loadTexture("models/trans2.png")
        }
      };
    ...
    App.prototype.getFresnelShader = function() {
      var mat, shader;
      ...
      mat.envMap = shader.uniforms["envMap"].value = this.skyCubeTexture;
      mat.lights = false;
    ...
    App.prototype.clean = function() {
      console.log("before", this.renderer.info.memory.programs);
      this.scene.remove(this.particles);
      this.renderer.deallocateMaterial(this.particles.material);
      return console.log("after", this.renderer.info.memory.programs);
    };

    App.prototype.cleanPlanes = function() {
      console.log("before", this.renderer.info.memory.programs);
    ...
    App.prototype.render = function() {
      var delta, exc, inv, mat, max, min, pl, pos, ran, time, _i, _len, _ref, _ref1;
      delta = this.clock.getDelta();
      this.renderer.clear();
      time = Date.now() * 0.0009;
      if (this.particles != null) {
        this.particles.rotation.y -= .01;
        /*
      ...
      if (this.planes != null) {
        ...
        this.planes.rotation.y -= .01;
        this.planes.updateMatrix();
        inv = mat.getInverse(this.planes.matrix);
        _ref = this.planes.children;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          pl = _ref[_i];
          ran = 1 + Math.sin(this.clock.oldTime / (500 * pl.growSpeed)) / 2;
          min = 0.5;
          max = 1.2;
          exc = max - min;
          ran = min + (ran * exc);
          pl.scale.set(ran, ran, 1);
          pl.lookAt(this.camera.position);
          pl.updateMatrix();
      ...
      if ((_ref1 = this.sph) != null) {
        _ref1.position.x = Math.sin(this.clock.oldTime / 1600) * 10;
      }
