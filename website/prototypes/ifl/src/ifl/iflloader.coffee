namespace "ifl"
	IFLLoader:
		class IFLLoader
			...
			constructor: ->
				@worker = new Worker 'js/libs/workers.js'
				...
				xhr.onreadystatechange = ()=>
					if xhr.readyState == xhr.DONE 
						...
						if @callbackProgress? 
							...
							@callbackProgress ({ 
								total: @length
								loaded: if response then response.length else NaN
							})
			...
			convertMesh: (iflmesh) ->
				...
				color_length = 0
				if colors? then color_length = iflmesh.verticesDecomposed._vertexAttributeLengths[ifl.IFLVertexAttribute.COLOR]
				# SKIN
			...
			convertBufferGeometry:(iflmesh, positions, uvs, normals, tangents, colors, color_length, isSkinnedMesh, skinWeights, skinIndices)->
				...
				geometry.materials = [ @convertMaterial( iflmesh.subMeshes[0] ) ]
				return geometry
			...
			convertGeometry:(iflmesh, positions, uvs, normals, tangents, colors, color_length, isSkinnedMesh, skinWeights, skinIndices)->
				...
				for subMesh,subMeshIndex in iflmesh.subMeshes
					# material
					material = @convertMaterial(subMesh)
					material.skinning = isSkinnedMesh
			...
			convertMaterial: (subMesh) ->
				...
				if !@matCache[matid]
					...
					@matCache[matid] =  new THREE.MeshPhongMaterial params 
