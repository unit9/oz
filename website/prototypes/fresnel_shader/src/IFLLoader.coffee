class IFLLoader
	...
	convertMesh: (iflmesh) ->
		...
		if positions.length < 65535 and !isSkinnedMesh and iflmesh.subMeshes.length == 1 and colors == null
	...
	convertMountainsMaterial:(subMesh)->
		params = 
			map:@getTexture("mountains.jpg2")
			fog: false

		return new THREE.MeshBasicMaterial(params)

	convertFresnelMaterial:(subMesh)->
		...
		params = @getMaterialParams(subMesh)
		# params.normalMap = THREE.ImageUtils.loadTexture "models/textures/normal.jpg"
		...
		#refractionIndexRatio =  1.0
		#fresnelFactor = Math.pow(1.0-refractionIndexRatio, 2.0)  / Math.pow(1.0+refractionIndexRatio, 2.0)
		# console.log "fresnel factor for IOR #{refractionIndexRatio} is #{fresnelFactor}"
		#uniforms[ "mFresnelFactor" ].value = fresnelFactor; 
		# uniforms[ "mFresnelFactor" ].value = 0.5
		# uniforms[ "useRefract" ].value = true


		# uniforms[ "offsetRepeat" ].value = new THREE.Vector4( 0, 0, 1, 1 )

		# lightex = @textureLoader.getTexture "test2_lightMap.jpg"
		# lightex = THREE.ImageUtils.loadTexture "models/textures/test_linear.png" 
	...
	convertTerrainMaterial:(subMesh)->
		...
		# uniforms[ "specular" ].value = 		new THREE.Color( subMesh.material.specular.uintValue )


		# lambert has only diffuse basically

		blendMap = @getTexture("terrain_blend.jpg3")

		# diffuse maps
		difftex  =  @getTexture("terrain_base.jpg3")
		lightex  =  @getTexture("terrain_lightmap.jpg3")
		difftexR =  @getTexture("terrain_diffuseR.jpg3")
		difftexG =  @getTexture("terrain_diffuseG.jpg3")
		difftexB =  @getTexture("terrain_diffuseB.jpg3")
