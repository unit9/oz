class IFLTornadoShaderWin
    ...
    fragmentShader: [
        ...
        "#define BENT 1"
        "#define FAKE_DIRECTIONAL_LIGHT 1"
        "#define ENVIRONMENT_TEXTURE 0"
        "#define NEGATE_Z 1  /// set to 1 for production viewer to match opengl convention"
        "#define USE_BOUNDING_VOLUME 1"

        "const float cam_fov=60.0;"
        "const float pi=3.141592654;"

        "const float tornado_height=80.0;/// how tall is the tornado"
        ...
        "const float tornado_bounding_radius=30.0; /// = max(pow(tornado_height,1.5)*0.03, bend_displacement)+10;"

        "uniform float environment_rotation; // 0 is 0 degrees, 0.5 is 180 degrees, 1 is 360 degrees"
        ...
        "const float base_step_scaling=0.9;/// Larger values allow for faster rendering but cause rendering artifacts. When stepping the isosurface, the value is multiplied by this number to obtain the distance of each step"
        ...
        "vec2 TextureCoord(vec3 q){"
            "#if BENT"
                "q.x+=sigmoid(1.0-q.z/bend_height)*bend_displacement;"
            "#endif"
             "return vec2(atan(q.y,q.x)*(0.5/pi)+environment_rotation, 1.0-q.z/tornado_height);"
        ...
        "{"
            ...
            "//vec3 col=color*brightness+multiplier*vec3(0.05,0.07,0.2);"
            "//gl_FragColor=vec4(col , 1.0);"
            "vec3 col = pow(color, vec3(gamma_correction))*final_colour_scale; //ContrastSaturationBrightness(color, 0.9, 1.4); // Adjust contrast / brightness"
            # "float a = 1.0 - (multiplier.r);"
            "float a = 1.01 - 1.01*multiplier.r;"
