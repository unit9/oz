class IFLLoader
    ...
    url                 : null
    loadingPhase        : 0
    totalLoadingPhases  : 3
    sky                 : null
    
    customMaterialInstancer : null
    ...
    handleProgress:(loaded,total)->
        if @callbackProgress?
            ...
            # console.log "#{@loadingPhase} : #{totalLoaded}"
    ...
    # loadAndCache: (url,callback,callbackProgress,xhrCallback,rawDataCallback) -> 
    #     @xhrCallback = xhrCallback
    #     @rawDataCallback = rawDataCallback
    #     @load(url,callback,callbackProgress)

    # decompressCachedLibrary: (url,callback,callbackProgress,response) -> 
    #     @loadingPhase = 0
    #     @url = url
    #     @callback = callback
    #     @callbackProgress = callbackProgress
    ...
    # instantiateCachedLibrary: (url,callback,callbackProgress,data) ->
    #     @loadingPhase = 2
    #     @url = url
    #     @callback = callback
    #     @callbackProgress = callbackProgress
    ...
    #     console.log "loader parseCachedLibrary"
    #     setTimeout(@doParseLibrary,1000)
    #     ###
    ...
    onXHRReadyStatusChange:()=>
        ...
        if @xhr.readyState == @xhr.DONE 
            if @xhr.status == 200 || @xhr.status == 0 
                ...
                @xhrCallback(response) if @xhrCallback? 
                @xhrCallback = null
                @decompressLibrary( response )
            else 
    ...
    decompressLibrary:(data) ->
        return if @disposed
        if !data? || data.length == 0 then throw "Error Loading File, length is 0"
        @t = new Date().getTime()
        @loadingPhase++
    ...
    parseLibrary:(data) ->
        return if @disposed
        if !data? || data.length == 0 then throw "Error Decompressing Library, length is 0"

        time = (new Date().getTime() - @t)/1000
        if @debugWarnings then console.log "[#{@url}] decompression time: #{time} , raw data length: #{data.length}" 
        ...
        # delay call because sometimes workers crash with huge amount of data
        @timer = setTimeout(@doParseLibrary,500)
    ...
    doParseLibrary:()=>
        ...
        @worker?.postMessage {type:"convert_library",data:@rawData,callback:"onLibraryParsed"}
    ...
    onLibraryParsed:(data) ->
        return if @disposed
        if !data? then "Error Parsing library, is null"

        @library = data;
        # @rawDataCallback(data) if @rawDataCallback?
        # @rawDataCallback = null
        
        time = (new Date().getTime() - @t)/1000
        if @debugWarnings then console.log "[#{@url}] Library Parse time: #{time}" 
    ...
    onTextureConverted: (data) ->
        return if @disposed
        bmp = @library._content[@convertTextureIndex-1]
        if !data?
            throw "Bitmap #{bmp._reference.id} contains invalid data"

        # console.log "parsed #{bmp._reference.id}, data length: #{data.length}"
        bmp.converted = data;
        @convertTextures();
        return null

    createModel:(skipCallBack) ->
        return if @disposed


        if @debugWarnings then console.log("[#{@url}] convert textures time: "+ (new Date().getTime() - @t)/1000  ) 
        ...
        if @debugWarnings then console.log("[#{@url}] convert node time: "+ (new Date().getTime() - @t)/1000  ) 
    ...
    convertMatrix4: (m) ->
        return new THREE.Matrix4(  m[0],   m[4],   m[8],   m[12],
                            m[1],   m[5],   m[9],   m[13],
                            m[2],   m[6],   m[10],  m[14],
                            m[3],   m[7],   m[11],  m[15]
                            );
    ...
    convertMesh: (iflmesh) ->
        ...
        if !uvs?
            if @debugWarnings then console.warn "Mesh #{iflmesh._reference.id} has no UVs"

        #seconduv
        seconduvs = iflmesh.verticesDecomposed._data[@IFLVertexAttribute.SECONDARY_UV]
        ...
        if !normals?
            if @debugWarnings then console.warn "Mesh #{iflmesh._reference.id} has no Normals"                 
        # TANGENTS
        ...
        if positions.length < 65535 and !isSkinnedMesh and iflmesh.subMeshes.length == 1 and !@pickableObjects[iflmesh._reference.id] and !seconduvs?
            geometry = @convertBufferGeometry iflmesh, positions, uvs, normals, tangents, colors, color_length, isSkinnedMesh, skinWeights, skinIndices, seconduvs
        else
            geometry = @convertGeometry iflmesh, positions, uvs, normals, tangents, colors, color_length, isSkinnedMesh, skinWeights, skinIndices, seconduvs

        geometry.hasTangents = tangents?
        @geometryLib.push geometry
        ...
        if @pickableObjects[iflmesh._reference.id] == true
            ret.pickable = true
        return ret

    convertBufferGeometry:(iflmesh, positions, uvs, normals, tangents, colors, color_length, isSkinnedMesh, skinWeights, skinIndices, seconduvs)->
        ...
        mat = @convertMaterial( iflmesh.subMeshes[0],iflmesh._reference.id )
        # if colors? then mat.vertexColors = THREE.VertexColors

        geometry.materials = [ mat ]
        return geometry
        

    convertGeometry:(iflmesh, positions, uvs, normals, tangents, colors, color_length, isSkinnedMesh, skinWeights, skinIndices, seconduvs)->
        
        #console.log "convert geom"
        ...
        if @debugWarnings then console.warn "[#{@url}] Warning: [#{iflmesh._reference.id}] is not using BufferGeometry (faster method) because #{reasons}"
        ...
        if positions
            for i in [0...positions.length] by 3
                ...
                if isSkinnedMesh
                    ...
                    if radiusSq > maxRadiusSq then maxRadiusSq = radiusSq;
                    ...
                    if !cachedPositions[p1+"_"+p2+"_"+p3]
                        ...
                        cachedPositions[p1+"_"+p2+"_"+p3] = {index: geometry.vertices.length,vertex:positionVec }
        ...
        # console.log "RADIUS OF #{iflmesh._reference.id} is #{maxRadiusSq}"
        ...
        for subMesh,subMeshIndex in iflmesh.subMeshes
            ...
            material.skinning = isSkinnedMesh
            # if colors? then material.vertexColors = THREE.VertexColors
            ...
            for i1,ibi in ib by 3
                ...
                primaryUVIndex = if switchUVs then 1 else 0
                secondaryUVIndex = if switchUVs then 0 else 1
                ...
                if seconduvs?
                    ...
                    fvUV2.push if cachedSecondUVs[i12] then cachedSecondUVs[i12] else cachedSecondUVs[i12] = new THREE.UV seconduvs[i12], seconduvs[i12+1]
                    fvUV2.push if cachedSecondUVs[i22] then cachedSecondUVs[i22] else cachedSecondUVs[i22] = new THREE.UV seconduvs[i22], seconduvs[i22+1]
                    fvUV2.push if cachedSecondUVs[i32] then cachedSecondUVs[i32] else cachedSecondUVs[i32] = new THREE.UV seconduvs[i32], seconduvs[i32+1]
        ...
        try geometry.computeCentroids() catch e
             if @debugWarnings then console.warn "[#{@url}] error computing centroids for #{iflmesh._reference.id}" 
    ...
    convertMaterial: (subMesh,meshname) ->
        ...
        if !@matCache[matid] || !@enableMaterialCache

            if @customMaterialInstancer?
                instanced = @customMaterialInstancer.apply(this,[subMesh,meshname])
                if instanced?
                    @matCache[matid] = instanced
                else
    ...
    getMaterialParams:(subMesh)->
        params = 
            ...
            normalMap: @getSubmeshTexture(subMesh.normalTextures)
            specularMap: @getSubmeshTexture(subMesh.specularTextures)
            lightMap: null
            bumpMap: null
            #properties
            reflectivity: subMesh.material.reflectivity
            shininess: subMesh.material.shininess
            opacity: if subMesh.material.transparency != 0 and subMesh.material.transparency != 1 then 1-subMesh.material.transparency else 1
            # wireframe: false
    ...
    getTexture: (id) ->
        ...
        if !tex?
            if @debugWarnings then console.warn "[#{@url}] Texture #{id} not found"
            return null

        if tex.converted?

            if !@texCache[tex._reference.id]? || !@enableTextureCache
            ...
            if @debugWarnings then console.warn "[#{@url}] Texture #{id} was not converted"   
    ...
    disabledGeometryAttributes : []
    disabledGeometryAttributesValues : []

    geometryAttributeEnabled:(attrib,value)=>

        if !value
            @disabledGeometryAttributes = []
            @disabledGeometryAttributesValues = []

            for geom in @geometryLib
                # buffergeometry
                if geom.attributes?[attrib]?
                    @disabledGeometryAttributes.push geom
                    @disabledGeometryAttributesValues.push geom.attributes[attrib]
                    geom.attributes[attrib] = null
                    geom[@getNeedsUpdateFlag(attrib)] = true
        else
            for geom,index in @disabledGeometryAttributes
                geom.attributes[attrib] = @disabledGeometryAttributesValues[index]
                geom[@getNeedsUpdateFlag(attrib)] = true

        return null

    getNeedsUpdateFlag:(attrib)=>
        switch attrib
            when "colors" then return "colorsNeedUpdate"
            when "vertices" then return "verticesNeedUpdate"
            when "uv" then return "uvsNeedUpdate"
            when "uv2" then return "uvsNeedUpdate"
            when "normals" then return "normalsNeedUpdate"
        return null

    dispose:()->
        @worker?.onmessage = ()-> console.log "[#{@url}] Message aborted"
        try @xhr?.abort()
